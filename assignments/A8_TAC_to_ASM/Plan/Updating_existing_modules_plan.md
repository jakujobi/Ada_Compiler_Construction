...
### Changes Needed:
    * **Calculate & Store Procedure Sizes:** When analyzing a procedure declaration (`PROCEDURE Name ... IS ... BEGIN ... END Name;`), traverse the declarations and the body:
        ...
        4.  Store the total calculated `SizeOfLocals` (sum of declared locals **plus** space for temps) into the `SymbolTable` entry's `local_size` field for `Name`. Calculate and store the total `SizeOfParams` into the `param_size` field.
    * **Initiate String Literal Handling:** While parsing `put`/`putln` arguments (or during semantic checks on them), if a string literal is identified, this module (or the Parser action calling it) should invoke the mechanism to store the literal (as `EntryType.CONSTANT`, label as name, `Value$` in `const_value`) and get its label from the `SymbolTable`.
...
### Verification:
    ...
    * Verify that the `SymbolTable` entries for procedures correctly store the calculated sizes in the `local_size` and `param_size` fields.
    * Verify that processing `put` statements with string literals results in appropriate `CONSTANT` entries being created in the `SymbolTable` mapping labels to `$`-terminated values in `const_value`.

### Module: `SymbolTable.py`

### Changes Needed (Implementation & Verification):
    * **Implement/Verify Procedure Size Fields:** Ensure procedure entries include fields `local_size: int` and `param_size: int`. *(Done: `local_size` exists, `param_size` added)*
    * **Implement/Verify String Literal Storage:** Ensure a mechanism exists to store string literals associated with labels. *(Done: Use `EntryType.CONSTANT`, label as name, value in `const_value`)*
    * **Implement/Verify Parameter Flag:** Ensure a clear way exists to distinguish parameters. *(Done: Use `EntryType.PARAMETER`)*
    * **Verify Offset Representation:** Review offsets. *(Done: Confirmed sufficient info stored)*
### Verification:
    * Write comprehensive unit tests for `SymTable.py`.
    * Test storing/retrieving `local_size` and `param_size`.
    * Test inserting/retrieving string literals via the `CONSTANT` entry mechanism.
    * Test lookup distinction using `EntryType.PARAMETER`.
